...
class L1Controller
{
    public:
        ...
        nav_msgs::Path Trans(const geometry_msgs::Point& wayPt, const geometry_msgs::Pose& carPose);
        double integral_function(const nav_msgs::Path& trans);
        void slow_down(const nav_msgs::Path& trans,double num_integeral);
...
nav_msgs::Path Trans(const geometry_msgs::Point& wayPt, const geometry_msgs::Pose& carPose,double eta)
{
    // geometry_msgs::Point odom_car2WayPtVec = get_odom_car2WayPtVec(carPose);//输入车坐标（以及规划的路径信息）返回iu可行的目标向量
    nav_msgs::Path trans;
    // double eta = atan2(odom_car2WayPtVec.y,odom_car2WayPtVec.x);//根据目标向量返回方位角（与x轴） 单位是弧度
    int i;
    float car2wayPt_x = wayPt.x - carPose.position.x;//从车到路径点的x y轴向距离
    float car2wayPt_y = wayPt.y - carPose.position.y;
    float distance;
    distance = sqrt(car2wayPt_x*car2wayPt_x + car2wayPt_y*car2wayPt_y);
    trans.poses[i].pose.position.x = distance*cos(eta);
    trans.poses[i].pose.position.y = distance*sin(eta);
    ROS_INFO("x = %.2f\n y = %.2f",trans.poses[i].pose.position.x,trans.poses[i].pose.position.y);
    return trans;
}
double L1Controller::integral_function(const nav_msgs::Path& trans)
{
    ...
    for (j=1;j<100;j++)//100一定需要修改
    {
        num_integeral = num_integeral + 0.5*(trans.poses[j].pose.position.x + trans.poses[j-1].pose.position.x)*(trans.poses[j].pose.position.y - trans.poses[j-1].pose.position.y);
       
    }
    num_integeral = abs(num_integeral);
    standard = abs(trans.poses[99].pose.position.x*trans.poses[99].pose.position.y*0.5);
    err = abs(standard-num_integeral);

    ROS_INFO("num_integeral: %f", num_integeral);
    ROS_INFO("standard: %f", standard);
    ROS_INFO("err: %f", err);
    return err;
}
